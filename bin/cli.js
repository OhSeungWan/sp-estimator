...
function getSlicePointAdvanced(slice, config, verbose = false) {
    ...
    const dependencyPenalty = Math.min((slice.dependencyCount || 0) * config.dependencyWeight, config.maxDependencyPenalty);
    ...
    const businessBonus = slice.isBusinessCritical
        ...
        : 0;
    const result = Math.ceil(basePoint * multiplier +
        dependencyPenalty +
        sharedBonus +
        businessBonus +
        layerWeight);
...
function calculateStoryPointAdvanced(slices, hasTest, isRefactor, config, verbose = false) {
    const sliceTotal = slices.reduce((sum, s) => sum + getSlicePointAdvanced(s, config, verbose), 0);
...
async function main() {
    ...
    while (addMore) {
        ...
        const { isSharedAcrossFeatures, dependencyCount, isBusinessCritical } = await inquirer.prompt([
            {
                type: "confirm",
    ...
    const result = calculateStoryPointAdvanced(slices, hasTest, isRefactor, config, verbose);
    console.log(`\n✅ 예상 스토리포인트: ${result} SP\n`);
}
